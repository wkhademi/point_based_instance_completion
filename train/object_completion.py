...
from utils.log_utils import log_losses, log_visualizations
...
def train(
    ...
    logger.info("Training object-level completion model...")
    ...
    for epoch in range(start_epoch, cfg.num_epochs):
        ...
        for data in tqdm(train_dataloader, total=len(train_dataloader)):
            partials = [partial.cuda() for partial in data["partial"]]
            partial_normals = [normals.cuda() for normals in data["partial_normals"]]
            gt_completions = data["complete"].cuda()
            gt_normals = data["complete_normals"].cuda()
            gt_center = data["object_center"].cuda()

            # produce completions
            pred_completions, pred_normals, pred_center = model(partials, partial_normals)
            ...
            if pred_center is not None:
                ...
                L_center = torch.tensor(0., device=gt_center.device)
    ...
    logger.info("Finished training object-level completion model.")
...
def validate(
    ...
    logger.info("Evaluating object-level completion model on validation set...")
