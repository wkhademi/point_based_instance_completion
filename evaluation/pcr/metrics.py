    ...
    for d in range(nd):
        ...
        if len(points_gt) > 0:
            ...
            for j in range(len(points_gt)):
                ...
                p = points_gt[j].astype(np.float32)
                v = np.array(mesh_pred.vertices, dtype=np.float32)
                f = np.array(mesh_pred.faces, dtype=np.int32)
                distance, _, _ = pcu.closest_points_on_mesh(p, v, f)
