...
os.environ["PATH"] += os.pathsep + os.path.join(os.path.realpath(os.path.curdir), "libs")
...
def eval(gt_dir, pred_dir, voxel_size, threshs=[0.25, 0.5]):
    ...
    for sid, scene_name in enumerate(scene_names):
        ...
        for (f, M) in gt_files_scene:
            label, mesh = load_gt_mesh(f, M)
            gt_meshes.append(mesh)
            gt_bboxes.append(mesh.bounds.reshape(-1))
            gt_labels.append(label)
        
        gt_valid_mask = np.ones(len(gt_meshes))[None, :]  # [B, M]
        gt_labels = np.array(gt_labels)[None, :]  # [B, M], in CAD ids [0, 33]
        gt_meshes = [gt_meshes]  # [B, M]
        gt_bboxes = np.array(gt_bboxes)[None, :]  # [B, M, 6]

        # voxelize ground truth meshes
        info_mesh_gts = batched_prepare_gt(
            gt_valid_mask, gt_labels, gt_bboxes, gt_meshes, voxel_size
        )
        ...
        for f in pred_files_scene:
            label, mesh, score = load_pred_mesh(f)
        ...
        pred_labels = np.array(pred_labels)[None, :]  # [B, M], in CAD ids [0, 33]
        pred_meshes = [pred_meshes]  # [B, M]
        pred_bboxes = np.array(pred_bboxes)[None, :]  # [B, M, 6]
        pred_scores = np.array(pred_scores)[None, :]  # [B, M]

        # voxelize predicted meshes
        info_mesh_preds = batched_prepare_pred(
            pred_valid_mask, pred_labels, pred_bboxes, pred_scores, pred_meshes, voxel_size
        )
...
if __name__ == '__main__':
    ...
    eval(args.data_dir, args.pred_dir, args.voxel_size)