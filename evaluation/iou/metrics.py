...
def prepare_pred(m, b, labels, bboxes, scores, meshes, voxel_size):
    ...
    voxels_internal = binvox.voxelize_mesh(mesh, dimension=resolution, wireframe=True, dilated_carving=True, verbose=VERBOSE, export_type="ply")
    voxels_surface = binvox.voxelize_mesh(mesh, exact=True, dimension=resolution, verbose=VERBOSE, export_type="ply")
...
def batched_prepare_pred(valid_mask, labels, bboxes, scores, meshes, voxel_size=0.047):
    ...
    for b in range(B):
        ...
        for m in valid_indices:
            tmp = prepare_pred(m, b=b, labels=labels, bboxes=bboxes, scores=scores, meshes=meshes, voxel_size=voxel_size)
            res.append(tmp)
...
def batched_prepare_gt(valid_mask, labels, bboxes, meshes, voxel_size=0.047):
    ...
    for b in range(B):
        ...
        for m in valid_indices:
            tmp = prepare_gt(m, b=b, labels=labels, bboxes=bboxes, meshes=meshes, voxel_size=voxel_size)
            res.append(tmp)
